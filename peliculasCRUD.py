...
def insertar_pelicula(titulo, director, año, genero, calificacion):
    messagebox.showinfo("Insertar", f"Insertar película: {titulo}, {director}, {año}, {genero}, {calificacion}")
...
if __name__ == "__main__":
    ...
    btn_insertar = ctk.CTkButton(frame_botones, text="Insertar", width=100, command=lambda: insertar_pelicula(entry_titulo.get(), entry_director.get(), entry_año.get(), entry_genero.get(), entry_calificacion.get()))
    btn_insertar.grid(row=0, column=0, padx=5, pady=5)

    btn_mostrar = ctk.CTkButton(frame_botones, text="Mostrar", width=100, command=mostrar_peliculas)
